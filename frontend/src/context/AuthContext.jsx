...
export function AuthProvider({ children }) {
  ...
  // ✅ NEW — update profile via backend
  const updateUserProfile = useCallback(async (data) => {
    try {
      const res = await api.put('/users/profile', data);

      if (res.data?.data) {
        setUserProfile((prev) => ({ ...prev, ...res.data.data }));
      }
  ...
  const value = useMemo(() => ({
    ...
    logout,
    resetPassword,
    updateProfile: updateUserProfile,   // ✅ exposed as updateProfile to consumers
    setUserProfile,
  }), [currentUser, userProfile, isAdmin, loading, register, login, logout, resetPassword, updateUserProfile]);
