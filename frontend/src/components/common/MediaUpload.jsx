...
function MediaUpload({ 
  ...
  type = 'image',
  folder = 'sermons',
  showUploadButton = false, // ✅ NEW: Always show upload button
  buttonText = 'Upload Media', // ✅ NEW: Custom button text
  multiple = false // ✅ NEW: Multiple files
}) {
  const [uploading, setUploading] = useState(false);
  const [preview, setPreview] = useState(currentMedia || null);
  const [mediaType, setMediaType] = useState(type);
  ...
  const handleMultipleFiles = async (files) => {
    ...
    setUploading(false);
    if (urls.length > 0) {
      onUpload(urls); // ✅ Send array of URLs
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={handleYoutubeSubmit}
              className="bg-church-gold text-church-navy px-4 py-2 rounded-lg font-semibold hover:bg-opacity-90 transition-colors"
            >
              Add Video
            </button>
          </div>
      ...
      {/* ✅ Upload Area - Always shows if showUploadButton is true OR no preview */}
      {(showUploadButton || !preview) && (
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-church-gold transition-colors">
          <label className="flex flex-col items-center justify-center cursor-pointer py-4">
            ...
            <span className="text-sm text-gray-500">
              {buttonText || `Click to upload ${mediaType}`}
            </span>
            <span className="text-xs text-gray-400 mt-1">
              {mediaType === 'video' 
                ? 'MP4, WEBM, MOV (max 100MB on Cloudinary)' 
            ...
            {mediaType === 'video' && (
              <span className="text-xs text-church-gold mt-1">
                Larger videos → Upload to YouTube
              </span>
            )}
            <input
              type="file"
      ...
      {/* ✅ Preview - Only show if preview exists AND showUploadButton is false (or for main media) */}
      {preview && !showUploadButton && renderPreview()}
