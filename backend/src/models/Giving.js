...
class Giving {
  ...
  static async getUserTotal(userId) {
    try {
      const results = await Database.getDocs(
        ...
        [
          { field: 'userId', operator: '==', value: userId },
          { field: 'status', operator: '==', value: 'successful' },
        ]
      );
  ...
  static async markSuccessfulFromVerification(id, verificationData, provider) {
    ...
    if (isSuccess) {
      ...
      return Database.updateDoc(COLLECTION, id, {
        status: 'failed',
        failedAt: new Date().toISOString(),
        failureReason: verificationData.gateway_response || 'Payment not successful',
        transactionData: verificationData,
        updatedAt: new Date().toISOString(),
      });
    }
