...
/**
 * ============================================================
 * ORDERS SCHEMA — FIELD MEANINGS
 * ============================================================
 *
 * Each order document has THREE fields related to "status."
 * They track different things and are all set independently.
 *
 * 1. `status` (top-level string)
 *    YOUR order lifecycle — where the order is in fulfillment.
 *    Values: pending | confirmed | processing | shipped
 *            | completed | cancelled | refunded
 *    Set by: POST /, webhook, PUT /:id/status, POST /:id/cancel
 *
 * 2. `paymentStatus` (top-level string)
 *    YOUR payment tracking — did money change hands?
 *    Values: pending | paid | failed | refunded
 *    Set by: POST / (cash), webhook, verify-payment
 *
 * 3. `paymentResponse.status` (nested string)
 *    THE PROVIDER'S status — raw payload from Paystack or Flutterwave.
 *    Paystack uses "success". Flutterwave uses "successful".
 *    Read-only — never write to this. It's audit data.
 *
 * Rule of thumb:
 *   - "Did they pay?"          → paymentStatus === 'paid'
 *   - "Has it shipped?"        → status === 'shipped' || 'completed'
 *   - "What did the gateway say?" → paymentResponse.status
 * ============================================================
 */
  ...
  try {
    ...
    if (paymentMethod === PAYMENT_METHODS.FLUTTERWAVE || paymentMethod === PAYMENT_METHODS.PAYSTACK) {
      try {
        ...
        logger.error(`Order ${orderNumber}: payment initialization failed`, {
        ...
        // Mark order so admin can see/retry
        await db.collection('orders').doc(docRef.id).update({
          status: 'payment_failed',
          paymentStatus: 'failed',
          paymentInitError: paymentError.message,
          updatedAt: FieldValue.serverTimestamp(),
        });
...
router.get('/user/:userId', authenticateUser, async (req, res) => {
  try {
    ...
    // Fetch user profile for role check (role lives in Firestore, not in the JWT)
    const userDoc = await db.collection('users').doc(req.user.uid).get();
    const userData = userDoc.exists ? userDoc.data() : {};
    const isAdmin = userData.role === 'admin';

    if (!isAdmin && req.user.uid !== userId) {
      return res.status(403).json({
        success: false,
        message: 'Access denied'
      });
...
router.post('/:id/cancel', authenticateUser, async (req, res) => {
  try {
    ...
    if (req.user.uid !== order.userId && !isAdmin) {
      return res.status(403).json({
        success: false,
        message: 'Access denied'
      });
...
router.get('/stats/overview', authenticateUser, requireAdmin, async (req, res) => {
  try {
    ...
    // Revenue = orders where payment was actually collected
    const totalRevenue = orders
